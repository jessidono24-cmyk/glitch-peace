...
export function drawTitle(ctx, w, h, backgroundStars, ts, menuIdx, selectedMode = 'grid') {
  ...
  const modeText = selectedMode === 'grid' ? '⊞ GRID MODE' : selectedMode === 'shooter' ? '◎ SHOOTER MODE' : selectedMode.toUpperCase();
  ctx.fillStyle = '#334455'; ctx.font = '10px Courier New'; 
  ctx.fillText('MODE: ' + modeText + ' (press M to change)', w / 2, h / 2 - 60);
  
  const menuTop = h / 2 - 40;
  ...
  ctx.fillStyle = '#131328'; ctx.font = '8px Courier New'; ctx.fillText('↑↓ navigate  ·  ENTER select  ·  M mode', w / 2, h - 20);
  ctx.textAlign = 'left';
}
